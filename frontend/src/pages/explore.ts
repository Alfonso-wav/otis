...
type ExploreTab = "typeChart" | "regions" | "moves" | "abilities";

let initialized = false;
let activeTab: ExploreTab = "typeChart";
...
function tabLabel(tab: ExploreTab): string {
  const icons: Record<ExploreTab, string> = { typeChart: ICON_CHART, regions: ICON_MAP, moves: ICON_BOLT, abilities: ICON_SPARKLES };
  return `${icons[tab]} ${t(`explore.tabs.${tab}`)}`;
}

const TAB_KEYS: ExploreTab[] = ["typeChart", "regions", "moves", "abilities"];
